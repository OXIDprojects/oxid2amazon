...
<tr>
	<td valign="top" class="edittext" width="25%">
        ...
        [{foreach from=$cattree item=oCat key=sCatId}]        	
        	<option value="[{ $oCat->getId() }]" [{if $sCatId|in_array:$aFilter.categories}]selected[{/if}]>[{ $oCat->oxcategories__oxtitle->rawValue }]</option>
        [{/foreach}]        
        </select>
        <br/><br/>
    	...
    	<table>
    			...
    			<td>
    				<select class="select" name="aFilter[fields][[{$fld_count}]][field]" [{ $readonly}]>
			          [{ foreach from=$aArticleFields item=field}]
			            <option value="[{$field}]" [{if $field == $oField.field}]selected[{/if}]>[{ oxmultilang ident="AZ_AMZ_FILTER_FIELD_oxarticles."|cat:$field noerror=1}]</option>
			          [{/foreach}]
			        </select>
    			</td>
    			<td>
    		...
    		<tr>
    			...
    			<td>    			
    				<select class="select" name="aFilter[fields][[{$fld_count}]][field]" [{ $readonly}]>
			          [{ foreach from=$aArticleFields item=field}]
			            <option value="[{$field}]">[{ oxmultilang ident="AZ_AMZ_FILTER_FIELD_oxarticles."|cat:$field noerror=1 }]</option>
			          [{/foreach}]
			        </select>
    			</td>
    			<td>
